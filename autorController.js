...
exports.crearAutor = async (req, res) => {
    ...
    const sql = 'INSERT INTO autor (nombre, nacionalidad) VALUES ($1, $2) RETURNING id, nombre, nacionalidad';

    try {
        const result = await db.query(sql, [nombre, nacionalidad]);
        const autorCreado = result.rows[0];
...
exports.actualizarAutor = async (req, res) => {
    ...
    try {
        ...
        res.status(200).json({ mensaje: 'Autor actualizado exitosamente', id });
    } catch (error) {
...
exports.eliminarAutor = async (req, res) => {
    ...
    try {
        ...
        res.status(200).json({ mensaje: 'Autor eliminado exitosamente', id });
    } catch (error) {
